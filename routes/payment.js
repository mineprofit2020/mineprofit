...
router.get('/info', requireAuth, async (req, res) => {
  try {
    ...
    const recentPayments = await db.prepare('SELECT * FROM payments WHERE user_id = ? ORDER BY created_at DESC LIMIT 10').all(userId);

    const gateways = await db.prepare('SELECT * FROM payment_gateways WHERE enabled=1 ORDER BY created_at DESC').all();
    const tokens = await db.prepare('SELECT * FROM crypto_tokens WHERE enabled=1 ORDER BY created_at DESC').all();
...
router.post('/submit', requireAuth, async (req, res) => {
  try {
    ...
    if (method === 'upi' || method === 'bank') {
      ...
      const gw = await db.prepare('SELECT * FROM payment_gateways WHERE id = ?').get(gateway_id);
      ...
      const tok = await db.prepare('SELECT * FROM crypto_tokens WHERE symbol = ? AND enabled=1').get(token_symbol.toUpperCase());
